...
// O2 is an implementation of the MatchFinder interface
// that is comparable to Brotli level 2, but uses the overlap
// parsing strategy.
type O2 struct {
	...
	for s < end {
		...
		for {
			for i := m.Start; i < m.End-2 && i < len(q.history)-8; i++ {
				h := q.hash(binary.LittleEndian.Uint64(q.history[i:]))
				q.table[h] = uint32(i)
			}
...
func (q *O2) search(pos, min, max int) AbsoluteMatch {
...
func (*O2) hash(u uint64) uint32 {
	hash := (u << 24) * hashMul64
	return uint32(hash >> (64 - o2Bits))
}
